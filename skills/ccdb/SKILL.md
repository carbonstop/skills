...
首次接入 CLI 或本地 stdio MCP 时，默认推荐 device OAuth，API Key 作为用户主动选择的备选；不要把 OAuth 失败解释为自动切换 Key 的许可。复用已连接的 MCP 不要求重新登录。远程 HTTP MCP 使用宿主 OAuth 流程，不要求用户执行本地 device 命令；具体指引见 [接入说明](references/access.md)。明确配置的环境 Key 仍有优先级，不自动删除或改换用户凭证。
