---
name: ccdb
description: 使用 Carbonstop 碳阻迹 CCDB 查询、比较和选择碳排放因子（emission factors），适用于产品碳足迹（PCF / carbon footprint）、企业碳核算和生命周期评价（LCA）的因子匹配。当用户需要查找电力、燃料、材料或运输活动的排放因子，比较候选或核对单位、地区、年份、系统边界和来源时使用。基于实际查询结果回答，并保留接口返回的可点击 Carbon Agent 详情链接；不执行建模写入，不替代完整核算或合规审核。
---

# CCDB 排放因子查询与匹配

Carbonstop 碳阻迹 · 产品碳足迹 / LCA。用于需要实际因子数据的查询与匹配；仅解释通用概念且不需要查数据时，无需调用 CCDB。
